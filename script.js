...
function jsonToTitleCase(obj) {
    for (let key in obj) {
        if (typeof obj[key] === 'string') {
            ...
            jsonToTitleCase(obj[key]);
        }
    }
}

jsonToTitleCase(jsonData);

// To keep track of unique values for the filters
let uniqueValues = {
    forms: new Set(),
    uses: new Set(),
    wildlifeValue: new Set(),
    lightTolerances: new Set(),
    soilTolerances: new Set(),
    flowerColor: new Set(),
};
...
function populateTable(data) {
    ...
    data.forEach(plant => {
        ...
        row.insertCell().textContent = plant.family ?? "";
        row.insertCell().textContent = plant.scientificName ?? "";
        row.insertCell().textContent = plant.commonNames ?? "";
		row.insertCell().textContent = plant.forms?.join(', ') ?? "";
		row.insertCell().textContent = plant.uses?.join(', ') ?? "";
		row.insertCell().textContent = plant.wildlifeValue?.join(', ') ?? "";
		row.insertCell().textContent = plant.lightTolerances?.join(', ') ?? "";
		row.insertCell().textContent = plant.soilTolerances?.join(', ') ?? "";
		row.insertCell().textContent = plant.flowerColor?.join(', ') ?? "";
		row.insertCell().textContent = plant.floweringTime?.join(', ') ?? "";
    ...
    // After populating the table, populate the dropdown filter options
...
function applyFilters() {
    ...
    let maxHeightFilter = $("#maxHeightSlider").slider("value");
...
$(function() {
    ...
    $("#maxHeightSlider").slider({
        ...
        min: 0,
        max: maxHeight,
        value: maxHeight, // Default value
        slide: function(event, ui) {
            // You can display the selected value if needed
        }
    });
});
